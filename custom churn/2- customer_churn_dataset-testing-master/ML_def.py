...
def classification_rate_by_group(
    df, 
    group_col: str, 
    target_col: str, 
    id_col: str, 
    ...
    """
    Calculate the classification rate by group in a given DataFrame.
    ...
    Returns:
    - A dictionary with the classification rates by group.
    """
    ...
    return group_classification_rates
...
def plot_histograms(df: pd.DataFrame, column: Optional[Union[str, None]] = None, save_plots: bool = False, palette: str = 'magma',
                            ...
                            multiple: Literal['layer', 'dodge', 'stack', 'fill'] = 'layer', show = True) -> None:
    """
...
def get_x_y_TVT(df: pd.DataFrame, target: Optional[str] = None, test_size: float = 0.2, train_size: float = 0.8, valid_size: float = 0.12, shuffle: bool = True, random_state: int = 42) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
    """
    ...
    def split_data(df: pd.DataFrame, target: Optional[str], test_size: float, train_size: float, valid_size: float, shuffle: bool, random_state: int) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
        """Splits the data into training, validation, and test sets."""
        x, y = extract_features_and_target(df, target)
        x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=test_size, train_size=train_size, shuffle=shuffle, random_state=random_state)
        x_train, x_valid, y_train, y_valid = train_test_split(x_train, y_train, test_size=valid_size, train_size=(1 - valid_size), shuffle=shuffle, random_state=random_state)
